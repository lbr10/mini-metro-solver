...
def oneLine(network, waiting, s, lineNb, totalLines=8):
    ...
    while not stop and count < len(network.stations):
        ...
        costs = [float('inf') for _ in range(n + 2)]

        for k in range(n):
            if not deja_vu[k]:
                costs[k] = totalWeight(network, branch + [k])
        
        if count >= len(waiting) / totalLines:
            costs[n] = totalWeight(network, branch)
            costs[n + 1] = totalWeight(network, branch + branch[n - 1 : 0 : -1]) * 3 / 4
